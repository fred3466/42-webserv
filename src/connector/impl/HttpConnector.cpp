...
HttpConnector::HttpConnector() : harl(), ns(), _soListen(-1), connectorListener(NULL), config()
{
}
...
//void HttpConnector::signalHandler(int signum)
//{
//	std::cout << "*-*-*-*-*-*-*-*-*-*GoodBye from HttpConnector.\n";
////	delete configVector;
...
HttpConnector::HttpConnector(std::string ipStr, int port, Config *c)
: harl(), ns(), _soListen(-1), connectorListener(NULL)
{
	...
	if ((_soListen = socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0)) == -1)
	{
	...
	/*************************************************************/
	_pollfd = new pollfd();
	memset(_pollfd, 0, sizeof(pollfd));
	_pollfd->fd = _soListen;
...
struct pollfd* HttpConnector::getPollFd()
{
	return _pollfd;
}
//void HttpConnector::doPoll()
//{
...
//int HttpConnector::getFdTabSize()
//{
//	return fdTabSize;
//}
//
//pollfd** HttpConnector::getFdTab()
//{
//	return fdTab;
//}
