...
CGIHandlerPHP::CGIHandlerPHP() : harl(), responseBody(""), responseHeaders(), config()
{
}
...
void CGIHandlerPHP::feedEnv(char **envp, std::map<std::string, std::string> envMap)
{
	int i = 0;
...
//			TODO BUG!!!!
		...
		harl.trace("CGIHandlerPHP::executeCGIScript env script\n[%s=%s] => [%s]", key.c_str(), val.c_str(), envp[i]);
		i++;
	}
	envp[i] = NULL;
...
std::string CGIHandlerPHP::buildCommandLine(const char *argv[], std::string interpreterPath, std::string &scriptPath)
{
	std::string cmdLine;
	int i = 0;
...
void CGIHandlerPHP::_childProcess(fdpipe *pipes, std::map<std::string, std::string> envMap, std::string interpreterPath, std::string &scriptPath, Request *request, char **envp)
{
	const char *argv[10];
	std::string cmdLine = buildCommandLine(argv, interpreterPath, scriptPath);
	...
	|| close(pipes->fd_for_RESPONSE[0]) == -1 || close(pipes->fd_for_RESPONSE[1]) == -1
		...
		throw std::runtime_error("CGIHandlerPHP::_childProcess dup2() of close() failed");
...
std::string
CGIHandlerPHP::executeCGIScript(std::string interpreterPath, std::string &scriptPath, Request *request,
		Response *response)
{
	std::string output;
...
//		harl.debug("CGIHandlerPHP::executeCGIScript: output :\n%s", output.c_str());
		return output;
	}
	int status;
...
void CGIHandlerPHP::_parentProcess(std::string *output, fdpipe *pipes, Request *request, int pid)
{
...
//		int written = write(pipes->fd_for_RESPONSE[1], cstr, length);
		(void) written;
//			harl.debug("CGIHandlerPHP::executeCGIScript: %d sent %d bytes through the wire", pipefd_pere_fils[1], written);
	}
	close(pipes->fd_for_REQUEST[1]);
	close(pipes->fd_for_RESPONSE[1]);
//	sleep(1);
