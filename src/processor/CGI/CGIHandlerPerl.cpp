...
CGIHandlerPerl::CGIHandlerPerl() : harl(), responseBody(""), responseHeaders(), config()
{
}
...
void CGIHandlerPerl::feedEnv(char **envp, std::map<std::string, std::string> envMap)
{
	int i = 0;
...
std::string CGIHandlerPerl::buildCommandLine(const char *argv[], std::string interpreterPath, std::string &scriptPath)
{
	std::string cmdLine;
	int i = 0;
...
void CGIHandlerPerl::_childProcess(fdpipe *pipes, std::map<std::string, std::string> envMap, std::string interpreterPath, std::string &scriptPath, Request *request, char **envp)
{
	const char *argv[4] =
			{ };
...
//		Contains the current script's path. This is useful for pages which need to point to themselves. The __FILE__ constant contains the full path and filename of the current (i.e. included) file.
	std::string scriptPathRoot = FileUtil().realPathFile(config->getParamStr("base_path", "base_path_missing"));
...
std::string
CGIHandlerPerl::executeCGIScript(std::string interpreterPath, std::string &scriptPath, Request *request,
		Response *response)
{
	std::string output;
...
void CGIHandlerPerl::_parentProcess(std::string *output, fdpipe *pipes, Request *request, int pid)
{
